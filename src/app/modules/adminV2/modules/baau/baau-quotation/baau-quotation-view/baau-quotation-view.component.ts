...
@Component({
  selector: 'app-baau-quotation-view',
  templateUrl: './baau-quotation-view.component.html',
  styleUrls: ['./baau-quotation-view.component.scss']
})
export class BaauQuotationViewComponent implements OnInit{
  ...
  viewDetails(){
    this.customerService.getbaauquotationdetailsByid(this.quotationId).subscribe((res:any)=>{
      if(res['success'] == true){
        ...
        let transactionId = this.viewData.transactionId;
        if (transactionId.startsWith('BC') || transactionId.startsWith('CU')) {
           
          this.priTran = true;
       
         }else if(transactionId.startsWith('BA')){
          this.secTran = true;
         }
    }else{
