import { Component, ViewChild } from '@angular/core';
import { MatTableDataSource } from '@angular/material/table';
import { Router } from '@angular/router';
import { GrantRequestsServices } from '../services/grant-requests-services.services';

@Component({
  selector: 'app-grant-gc-kyc',
  templateUrl: './grant-gc-kyc.component.html',
  styleUrls: ['./grant-gc-kyc.component.scss']
})
export class GrantGCKycComponent {
  constructor(private router: Router, private grantRequestServices: GrantRequestsServices) { }
  @ViewChild('customTable') customTable: any
  data = []
  tableStructure: any = {
    displayColumns: [
      'more',
      'User Id',
      'Entity Id',
      'Company Name',
      'First Name',
      'Last Name',
      'Email',
      'Mobile',
      'Business Status',
      'Personal Status',
      'Action'
      
    ],
    dataSource: new MatTableDataSource([]),
    // compareDate: true,

    page: {
      previousPageIndex: 1,
      pageIndex: 0,
      pageSize: 10,
      length: 0,
    },

    transaction: {
      showTransType: false,
      selectedTransactionType: 'MAKER_APPROVED',

    },

  };



  ngOnInit(): void {
    this.grantRequestServices.getSelectedTabIndex().subscribe((index: number) => {
      if (index === 7) { 
        let payload = {
          page: 0,
          size: 10,
          status: 'MAKER_APPROVED'
        }
        this.fetchData(payload);
      }
    });

  }


  tableEventEmitter(event) {
    console.log(event)
    switch (event.eventName) {
      case 'paginationButton':
        this.goNextPrevious(event.event)
        break;
      case 'entrySize':
        this.changeEntrySize(event.event)
        break;
      case 'approvePayment':
        this.onupdatePayment(event.event)
        break;
      case 'applicableEntType':
        this.changeapplicableEntityType(event.event)
        break;
    }
  }

  changeTransType(status: any) {
    let payload = {
      page: 0,
      size: 10,
      status: 'MAKER_APPROVED',
    }
    this.fetchData(payload);
  }

  changeapplicableEntityType(subscriberType: any){
    let payload = {
      page: 0,
      size: 10,
      status: 'MAKER_APPROVED',
    }
    this.fetchData(payload);
    if(this.tableStructure.applicableEntity.showapplicableEntityType == 'CUSTOMER' || this.tableStructure.applicableEntity.showapplicableEntityType == 'BANK' || this.tableStructure.applicableEntity.showapplicableEntityType == 'BANK_AS_UNDERWRITER'){
      this.tableStructure.displayColumns = ['User Id','First Name','Last Name','Email','Mobile','Plan and Payment','Action'];
    }else{
      this.tableStructure.displayColumns = ['User Id','First Name','Last Name','Email','Mobile','Action'];
    }
  }
  
  fetchData(payload: any) {
    this.grantRequestServices.getGCKycList(payload).subscribe({
      next: (response: any) => {
        this.tableStructure.page = {
          previousPageIndex: (response.page - 1),
          pageIndex: response.page,
          pageSize: response.size,
          length: response.totalElements,
        }
        let data = response.content.map((item: any) => {
          return {
            'User Id' : item?.personalDetails.username,
            'Entity Id' : item?.businessDetails?.entityId,
            'Company Name' : item?.businessDetails?.companyName,
            'First Name' : item?.personalDetails?.firstName,
            'Last Name' : item?.personalDetails?.lastName,
            'Email' : item?.personalDetails?.email,
            'Mobile' : item?.personalDetails?.mobileNumber,
            'Business Status': item?.kycDetails?.[0].documentList[0].kycStatus,
            'Personal Status': item?.kycDetails?.[1].documentList[0].kycStatus,
            'Action' : {  
              from : 'grant_gc_kyc',
              item: item
            }

          }
        })
        this.tableStructure.dataSource = new MatTableDataSource(data);
        this.customTable.refreshTable();
      }
    })
  }

  
  goNextPrevious(data) {
    let payload = {
      page: data.pageIndex,
      size: data.pageSize,
      status: 'MAKER_APPROVED',
    }
    this.fetchData(payload);
  }

  changeEntrySize(data) {
    let payload = {
      page: data.pageIndex,
      size: data.pageSize,
      status: 'MAKER_APPROVED',
    }
    this.fetchData(payload);
  }
  onupdatePayment(approvePayment){
    let payload = {
      page: 0,
      size: 10,
      status: 'MAKER_APPROVED',
    }
    this.fetchData(payload);
  }


}
