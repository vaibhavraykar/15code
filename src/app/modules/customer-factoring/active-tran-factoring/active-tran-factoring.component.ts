...
@Component({
  selector: 'app-active-tran-factoring',
  templateUrl: './active-tran-factoring.component.html',
  styleUrls: ['./active-tran-factoring.component.scss']
})
export class ActiveTranFactoringComponent {
  ...
  advanceRate: any;
  factoringCommission: any;
  interestRate: any;


//   selected = 'Interested';
  constructor(private store: Store,private router: Router,private custFactService:CustomerFactoringService,private dialog: MatDialog
    ){
      this.displayColumns=['Assignor_Seller','Debotr_Buyer','Amount','Invoice_Date','Due_Date','Status','Comment','Pricing','View'];
  ...
  pagePrevious() {
    ...
    let payload = {page: this.page.index, size: 5,status:'ACTIVE'};
    console.log(payload);
    this.getList(payload);
  }
  pageNext() {
  ...
  viewPricingDetails(data:any) {
    this.advanceRate = data.advanceRate;
    this.factoringCommission = data.factoringCommission;
